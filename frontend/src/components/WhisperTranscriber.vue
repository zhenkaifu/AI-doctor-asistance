...
执行步骤：
0. 角色区分：先识别文本中哪些话是医生说的（通常是问句或引导性语句），哪些是病人说的（主诉、症状描述、回答、既往史等）。忽略医生说的话，只提取病人说的内容用于后续处理。
1. 文本清洗：对病人说的内容进行清洗——修正明显错别字和同音误识；删除与病情无关或重复的信息；不编造未出现的病史与检查结果。
2. 基于清洗后的患者信息，判断是否**必须追问**才能做出合理临床决策。追问的门槛：当前信息缺失会直接影响诊断方向或患者安全（如危险信号、鉴别诊断的关键依据）。常见病、典型表现或已可合理推断的情况，不应追问，直接进入“不需要继续追问”分支。
